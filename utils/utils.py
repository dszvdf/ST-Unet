...
def correlation(image1, image2=None):
    ...
    image1 = torch.from_numpy(image1).type(torch.float32).to(device)
    if (image2 is None):
        # image1 = filter(image1, 2)
        Freq = abs(torch.fft.fft2(torch.fft.fftshift(image1))) ** 2
        corr = abs(torch.fft.ifftshift(torch.fft.ifft2(Freq)))
        corr = corr - corr.min()
        return corr
    else:
        Freq_1 = abs(torch.fft.fft2(torch.fft.fftshift(image1)))
        Freq_2 = abs(torch.fft.fft2(torch.fft.fftshift(image2)))
        Freq = Freq_1 * Freq_2
        corr = abs(torch.fft.ifftshift(torch.fft.ifft2(Freq)))
        corr = corr - corr.min()
        return corr
...
def preprocess_mnist(original_image, experiment_image, Size):
    ...
    compose = torchvision.transforms.Resize(size=(128, 128))
    corr = corr.unsqueeze(dim=0).unsqueeze(dim=0)
    corr = compose(corr)
    corr = 255 * corr / corr.max()
